...
class JSONLTextDataset(Dataset):
    """Custom Dataset for JSONL format with text data."""
    
    def __init__(self, jsonl_path, text_field, label_field, tokenizer, max_length, label2id):
        self.data = []
...
def main(args):
    ...
    train_dataset = JSONLTextDataset(
        args.train_path, args.text_field, args.label_field, 
        tokenizer, args.max_seq_length, label2id
    )
    dev_dataset = JSONLTextDataset(
        args.dev_path, args.text_field, args.label_field, 
        tokenizer, args.max_seq_length, label2id
    )
    test_dataset = JSONLTextDataset(
        args.test_path, args.text_field, args.label_field, 
        tokenizer, args.max_seq_length, label2id
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train text classifier on JSONL format data")
    ...
    parser.add_argument("--text_field", type=str, required=True, help="Field name containing text")
